...
class ExactMatchStrategy(ComparisonStrategy):
    ...
    def compare(
        ...
        return ComparisonResult(
            similarity_score=0.0, confidence=1.0, match_type="no_match", details={}
        )
...
class StructuralSimilarityStrategy(ComparisonStrategy):
    ...
    def compare(
        ...
        scores["type_similarity"] = (
            1.0 if entity1.entity_type == entity2.entity_type else 0.0
        )

        scores["property_similarity"] = self._compare_properties(
        ...
        scores["hierarchy_similarity"] = self._compare_hierarchy_position(
            entity1, entity2
        )
        ...
        match_type = (
            ...
            else "partial_structural_match" if overall_score > 0.3 else "no_match"
        )
    ...
    def _compare_hierarchy_position(
        self, entity1: TMFEntity, entity2: TMFEntity
    ) -> float:
        score = 0.0
        ...
        children_similarity = self._compare_relationships(
            entity1.children_ids, entity2.children_ids
        )
    ...
    def _compare_contexts(
        ...
        if neighbor_names1 or neighbor_names2:
            neighbor_similarity = (
                len(neighbor_names1 & neighbor_names2)
                / len(neighbor_names1 | neighbor_names2)
                if (neighbor_names1 | neighbor_names2)
                else 0.0
            )
        ...
        if ancestor_names1 or ancestor_names2:
            ancestor_similarity = (
                len(ancestor_names1 & ancestor_names2)
                / len(ancestor_names1 | ancestor_names2)
                if (ancestor_names1 | ancestor_names2)
                else 0.0
            )
...
class SemanticSimilarityStrategy(ComparisonStrategy):
    ...
    def compare(
        ...
        match_type = (
            ...
            else "partial_semantic_match" if overall_score > 0.4 else "no_match"
        )
    ...
    def _calculate_pattern_similarity(
        self, words1: Set[str], words2: Set[str]
    ) -> float:
