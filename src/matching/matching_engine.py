...
class TMFMatchingEngine:
    ...
    def match_ontologies(
        ...
        self.logger.info(
            f"Starting ontology matching: {source_ontology.name} -> {target_ontology.name}"
        )
    ...
    def _match_ontologies_parallel(
        self, source_kg: TMFKnowledgeGraph, target_kg: TMFKnowledgeGraph
    ) -> List[MatchingResult]:
        matches = []
        ...
        with ThreadPoolExecutor(max_workers=self.config.max_workers) as executor:
            future_to_entity = {
                executor.submit(
                    self._match_entity, entity, source_kg, target_kg
                ): entity
    ...
    def _match_ontologies_sequential(
        self, source_kg: TMFKnowledgeGraph, target_kg: TMFKnowledgeGraph
    ) -> List[MatchingResult]:
        matches = []
    ...
    def _match_entity(
        ...
        for target_entity in target_entities:
            ...
            if (
                comparison_result.similarity_score >= self.config.similarity_threshold
                and comparison_result.confidence >= self.config.confidence_threshold
            ):
    ...
    def _get_candidate_entities(
        self, source_entity: TMFEntity, target_kg: TMFKnowledgeGraph
    ) -> List[TMFEntity]:
        if self.config.filter_by_entity_type:
            candidates = target_kg.ontology.get_entities_by_type(
    ...
    def _rank_matches(self, matches: List[MatchingResult]) -> List[MatchingResult]:
        return sorted(
            matches, key=lambda m: (m.similarity_score, m.confidence), reverse=True
        )

    def match_entities(
        self, source_entities: List[TMFEntity], target_entities: List[TMFEntity]
    ) -> List[MatchingResult]:
        matches = []
        ...
        for source_entity in source_entities:
            for target_entity in target_entities:
                if (
                    self.config.filter_by_entity_type
                    and source_entity.entity_type != target_entity.entity_type
                ):
                    continue
...
class MatchingResultProcessor:
            ...
            if source_id not in mapping:
                ...
                existing_match_score = next(
                    (
                        m.similarity_score
                        for m in matches
                        if m.source_entity.id == source_id
                        and m.target_entity.id == mapping[source_id]
                    ),
                    0.0,
                )
    ...
    def export_matches_to_dict(self, matches: List[MatchingResult]) -> Dict[str, Any]:
        return {
            ...
            "metadata": {"total_matches": len(matches), "generation_time": time.time()},
        }
    ...
    def filter_matches_by_threshold(
        ...
        if similarity_threshold is not None:
            filtered = [
                m for m in filtered if m.similarity_score >= similarity_threshold
            ]
