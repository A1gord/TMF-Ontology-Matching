...
class MatchingEvaluator:
    ...
    def evaluate_matching_results(self, matching_results: List[MatchingResult],
                                ground_truth: List[GroundTruthMapping]) -> EvaluationMetrics:
    ...
    def _extract_predicted_mappings(self, matching_results: List[MatchingResult]) -> Set[Tuple[str, str]]:
        return {(result.source_entity.id, result.target_entity.id) for result in matching_results}
    
    def _extract_ground_truth_mappings(self, ground_truth: List[GroundTruthMapping]) -> Set[Tuple[str, str]]:
        return {(gt.source_id, gt.target_id) for gt in ground_truth if gt.is_correct}
    
    def evaluate_by_threshold(self, matching_results: List[MatchingResult],
                            ground_truth: List[GroundTruthMapping],
                            thresholds: List[float]) -> Dict[float, EvaluationMetrics]:
        
        results = {}
        
        for threshold in thresholds:
            filtered_results = [r for r in matching_results if r.similarity_score >= threshold]
            metrics = self.evaluate_matching_results(filtered_results, ground_truth)
            results[threshold] = metrics
        
        return results
    
    def evaluate_by_match_type(self, matching_results: List[MatchingResult],
                             ground_truth: List[GroundTruthMapping]) -> Dict[str, EvaluationMetrics]:
        
        results = {}
        match_types = set(result.match_type for result in matching_results)
        
        for match_type in match_types:
            filtered_results = [r for r in matching_results if r.match_type == match_type]
            metrics = self.evaluate_matching_results(filtered_results, ground_truth)
            results[match_type] = metrics
        
        return results
    
    def calculate_ranking_metrics(self, matching_results: List[MatchingResult],
                                ground_truth: List[GroundTruthMapping],
        ...
        for source_id in results_by_source:
            results_by_source[source_id].sort(key=lambda r: r.similarity_score, reverse=True)
    ...
    def _calculate_mean_reciprocal_rank(self, results_by_source: Dict[str, List[MatchingResult]],
                                      gt_mappings: Set[Tuple[str, str]]) -> float:
...
class AlignmentEvaluator:
    ...
    def evaluate_alignment_results(self, alignment_results: List[AlignmentResult],
                                 ground_truth: List[GroundTruthMapping]) -> EvaluationMetrics:
    ...
    def _extract_predicted_alignments(self, alignment_results: List[AlignmentResult]) -> Set[Tuple[str, str]]:
        return {(result.source_entity.id, result.target_entity.id) for result in alignment_results}
    
    def _extract_ground_truth_mappings(self, ground_truth: List[GroundTruthMapping]) -> Set[Tuple[str, str]]:
        return {(gt.source_id, gt.target_id) for gt in ground_truth if gt.is_correct}
    
    def evaluate_by_alignment_type(self, alignment_results: List[AlignmentResult],
                                 ground_truth: List[GroundTruthMapping]) -> Dict[str, EvaluationMetrics]:
        
        results = {}
        alignment_types = set(result.alignment_type.value for result in alignment_results)
        
        for alignment_type in alignment_types:
            filtered_results = [r for r in alignment_results 
                              if r.alignment_type.value == alignment_type]
            metrics = self.evaluate_alignment_results(filtered_results, ground_truth)
            results[alignment_type] = metrics
        
        return results
    
    def evaluate_hierarchical_consistency(self, alignment_results: List[AlignmentResult]) -> Dict[str, float]:
        ...
        consistency_rate = 1.0 - (consistency_violations / total_hierarchical_pairs) if total_hierarchical_pairs > 0 else 1.0
...
class ComprehensiveEvaluator:
    ...
    def evaluate_complete_pipeline(self, matching_results: List[MatchingResult],
                                 alignment_results: List[AlignmentResult],
                                 ground_truth: List[GroundTruthMapping]) -> Dict[str, Any]:
        ...
        hierarchical_metrics = self.alignment_evaluator.evaluate_hierarchical_consistency(
            alignment_results)
        ...
        return {
            ...
            "summary": {
                "overall_f1": (matching_metrics.f1_score + alignment_metrics.f1_score) / 2,
                "overall_precision": (matching_metrics.precision + alignment_metrics.precision) / 2,
                "overall_recall": (matching_metrics.recall + alignment_metrics.recall) / 2
            }
        }
