...
class RDFConverter:
    ...
    def ontology_to_rdf(self, ontology: TMFOntology) -> Graph:
        try:
            ...
            ontology_uri = URIRef(
                ontology.namespace or f"{default_base_uri}/{ontology.name}"
            )
            ...
            self.logger.info(
                f"Converted ontology '{ontology.name}' to RDF with {len(graph)} triples"
            )
            return graph
    ...
    def _add_entity_to_graph(
        self, graph: Graph, entity: TMFEntity, ontology_uri: URIRef
    ) -> None:
        ...
        for key, value in entity.properties.items():
            ...
            if isinstance(value, (int, float)):
                graph.add(
                    (entity_uri, property_uri, Literal(value, datatype=XSD.decimal))
                )
            elif isinstance(value, bool):
                graph.add(
                    (entity_uri, property_uri, Literal(value, datatype=XSD.boolean))
                )
            else:
                graph.add((entity_uri, property_uri, Literal(str(value))))

        graph.add(
            (entity_uri, self.tmf_ns.entityType, Literal(entity.entity_type.value))
        )
        graph.add(
            (
                entity_uri,
                self.tmf_ns.confidenceScore,
                Literal(entity.confidence_score, datatype=XSD.decimal),
            )
        )
        graph.add(
            (entity_uri, self.tmf_ns.sourceOntology, Literal(entity.source_ontology))
        )
    ...
    def rdf_to_ontology(self, graph: Graph, ontology_name: str = None) -> TMFOntology:
        try:
            ...
            if ontology_uris:
                ...
                name = (
                    str(graph.value(ontology_uri, RDFS.label))
                    or ontology_name
                    or "Imported_Ontology"
                )
            ...
            properties = list(graph.subjects(RDF.type, OWL.DatatypeProperty)) + list(
            ...
            self.logger.info(
                f"Converted RDF to ontology '{name}' with {len(ontology.entities)} entities"
            )
            return ontology
    ...
    def _create_entity_from_rdf(
        self, graph: Graph, entity_uri: URIRef, base_namespace: str
    ) -> Optional[TMFEntity]:
        try:
            ...
            for predicate, obj in graph.predicate_objects(entity_uri):
                if predicate.startswith(self.tmf_ns) and predicate not in [
                    self.tmf_ns.synonym,
                    self.tmf_ns.entityType,
                    self.tmf_ns.confidenceScore,
                    self.tmf_ns.sourceOntology,
                ]:
    ...
    def _infer_entity_type_from_rdf_type(
        ...
        try:
            ...
            for i, result in enumerate(results):
                ...
                graph.add(
                    (
                        match_uri,
                        self.matching_ns.similarityScore,
                ...
                graph.add(
                    (
                        match_uri,
                        self.matching_ns.confidence,
                        Literal(result.confidence, datatype=XSD.decimal),
                    )
                )
                graph.add(
                    (match_uri, self.matching_ns.matchType, Literal(result.match_type))
                )
                graph.add(
                    (
                        match_uri,
                        self.matching_ns.timestamp,
                ...
                if result.details:
                    ...
                    graph.add(
                        (match_uri, self.matching_ns.details, Literal(details_json))
                    )
        ...
        try:
            ...
            for i, result in enumerate(results):
                ...
                graph.add(
                    (
                        alignment_uri,
                        self.alignment_ns.alignmentType,
                ...
                graph.add(
                    (
                        alignment_uri,
                        self.alignment_ns.hierarchicalDistance,
                ...
                graph.add(
                    (
                        alignment_uri,
                        self.alignment_ns.structuralSimilarity,
                ...
                if result.details:
                    ...
                    graph.add(
                        (
                            alignment_uri,
                            self.alignment_ns.details,
                            Literal(details_json),
                        )
                    )
    ...
    def save_rdf_to_file(
        self, graph: Graph, file_path: str, format: str = None
    ) -> None:
        try:
            if format is None:
                format = "turtle"
                if self.config and hasattr(self.config, "serialization"):
                    ...
                    format = format_mapping.get(
                        self.config.serialization.default_format, "turtle"
                    )
...
class Neo4jToRDFConverter:
    ...
    def export_ontology_from_neo4j_to_rdf(
        self, ontology_name: str, output_path: str, format: str = "turtle"
    ) -> None:
        try:
            ...
            self.logger.info(
                f"Exported ontology '{ontology_name}' from Neo4j to RDF file '{output_path}'"
            )
    ...
    def export_matching_results_from_neo4j_to_rdf(
        self,
        ontology1_name: str,
        ontology2_name: str,
        output_path: str,
        ...
        try:
            ...
            importer = TMFNeo4jImporter(self.neo4j_connector)
            matching_results = importer.import_matching_results(
            ...
            self.logger.info(
                f"Exported matching results from Neo4j to RDF file '{output_path}'"
            )

        except Exception as e:
            self.logger.error(
                f"Error exporting matching results from Neo4j to RDF: {e}"
            )
            raise

    def export_alignment_results_from_neo4j_to_rdf(
        self,
        ontology1_name: str,
        ontology2_name: str,
        output_path: str,
        ...
        try:
            ...
            importer = TMFNeo4jImporter(self.neo4j_connector)
            alignment_results = importer.import_alignment_results(
                ontology1_name, ontology2_name
            )
            ...
            self.logger.info(
                f"Exported alignment results from Neo4j to RDF file '{output_path}'"
            )

        except Exception as e:
            self.logger.error(
                f"Error exporting alignment results from Neo4j to RDF: {e}"
            )
            raise
    ...
    def import_rdf_to_neo4j(self, rdf_file_path: str, ontology_name: str) -> None:
        try:
            ...
            self.logger.info(
                f"Imported RDF file '{rdf_file_path}' to Neo4j as ontology '{ontology_name}'"
            )
