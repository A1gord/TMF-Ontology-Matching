...
class TMFNeo4jExporter:
    ...
    def _create_relationships(self, ontology: TMFOntology) -> None:
        for entity in ontology.entities.values():
            if entity.parent_id:
                self._create_hierarchy_relationship(entity.id, entity.parent_id, ontology.name)
    ...
    def _create_hierarchy_relationship(self, source_id: str, target_id: str, ontology_name: str) -> None:
        query = """
    ...
    def _create_semantic_relationship(self, source_id: str, target_id: str, ontology_name: str) -> None:
        query = """
        MATCH (source:TMFEntity {id: $source_id, ontology_name: $ontology_name})
        MATCH (target:TMFEntity {id: $target_id, ontology_name: $ontology_name})
    ...
    def export_matching_results(self, results: List[MatchingResult], ontology1_name: str, ontology2_name: str) -> None:
        try:
            ...
            self.logger.info(f"Matching results exported to Neo4j for {ontology1_name} -> {ontology2_name}")
        except Exception as e:
    ...
    def _create_match_relationship(self, result: MatchingResult, ontology1_name: str, ontology2_name: str) -> None:
        query = """
    ...
    def export_alignment_results(self, results: List[AlignmentResult], ontology1_name: str, ontology2_name: str) -> None:
        try:
            self._clear_alignment_results(ontology1_name, ontology2_name)
            
            for result in results:
                self._create_alignment_relationship(result, ontology1_name, ontology2_name)
            
            self.logger.info(f"Alignment results exported to Neo4j for {ontology1_name} -> {ontology2_name}")
        except Exception as e:
            self.logger.error(f"Error exporting alignment results to Neo4j: {e}")
            raise
    
    def _clear_alignment_results(self, ontology1_name: str, ontology2_name: str) -> None:
        query = """
    ...
    def _create_alignment_relationship(self, result: AlignmentResult, ontology1_name: str, ontology2_name: str) -> None:
        query = """
        MATCH (source:TMFEntity {id: $source_id, ontology_name: $ontology1_name})
        MATCH (target:TMFEntity {id: $target_id, ontology_name: $ontology2_name})
...
class TMFNeo4jImporter:
    ...
    def _load_relationships(self, entities: List[TMFEntity], ontology_name: str) -> None:
        ...
        hierarchy_results = self.connector.execute_query(hierarchy_query, {"ontology_name": ontology_name})
        ...
        semantic_results = self.connector.execute_query(semantic_query, {"ontology_name": ontology_name})
    ...
    def import_matching_results(self, ontology1_name: str, ontology2_name: str) -> List[MatchingResult]:
        try:
            query = """
            MATCH (source:TMFEntity {ontology_name: $ontology1_name})-[r:MATCHES]->(target:TMFEntity {ontology_name: $ontology2_name})
            ...
            self.logger.info(f"Imported {len(matching_results)} matching results from Neo4j")
    ...
    def import_alignment_results(self, ontology1_name: str, ontology2_name: str) -> List[AlignmentResult]:
        try:
            ...
            self.logger.info(f"Imported {len(alignment_results)} alignment results from Neo4j")
    ...
    def get_ontology_statistics(self, ontology_name: str) -> Dict[str, Any]:
        try:
            ...
            stats_result = self.connector.execute_query(stats_query, {"ontology_name": ontology_name})
            relationships_result = self.connector.execute_query(relationships_query, {"ontology_name": ontology_name})
            
            statistics = {
                "ontology_name": ontology_name,
                "total_entities": stats_result[0]['total_entities'] if stats_result else 0,
                "entity_types": stats_result[0]['entity_types'] if stats_result else [],
                "relationships": {r['relationship_type']: r['count'] for r in relationships_result}
            }
