...
class XMLSerializer:
    ...
    def serialize_ontology(self, ontology: TMFOntology, file_path: str) -> None:
        try:
            ...
            if (
                self.config
                and hasattr(self.config, "serialization")
                and self.config.serialization.pretty_print
            ):
    ...
    def _create_rdf_root(self, ontology: TMFOntology) -> ET.Element:
        ...
        ontology_element = ET.SubElement(
            root, f"{{{self.namespace_map['owl']}}}Ontology"
        )
        ontology_element.set(
            f"{{{self.namespace_map['rdf']}}}about", ontology.namespace
        )

        version_element = ET.SubElement(
            ontology_element, f"{{{self.namespace_map['owl']}}}versionInfo"
        )
    ...
    def _create_entity_element(self, entity: TMFEntity) -> ET.Element:
        ...
        if entity.name:
            label_element = ET.SubElement(
                element, f"{{{self.namespace_map['rdfs']}}}label"
            )
            label_element.text = entity.name

        if entity.description:
            comment_element = ET.SubElement(
                element, f"{{{self.namespace_map['rdfs']}}}comment"
            )
            comment_element.text = entity.description

        if entity.parent_id:
            subclass_element = ET.SubElement(
                element, f"{{{self.namespace_map['rdfs']}}}subClassOf"
            )
            subclass_element.set(
                f"{{{self.namespace_map['rdf']}}}resource", entity.parent_id
            )

        for relationship_id in entity.relationships:
            related_element = ET.SubElement(
                element, f"{{{self.namespace_map['rdfs']}}}seeAlso"
            )
            related_element.set(
                f"{{{self.namespace_map['rdf']}}}resource", relationship_id
            )

        for key, value in entity.properties.items():
            prop_element = ET.SubElement(
                element, f"{{{self.namespace_map['tmf']}}}{key}"
            )
...
class UniversalSerializer:
    ...
    def deserialize_ontology(self, file_path: str) -> TMFOntology:
        ...
        if file_extension == ".json":
            ...
            raise ValueError(
                f"Deserialization not supported for format: {file_extension}"
            )
