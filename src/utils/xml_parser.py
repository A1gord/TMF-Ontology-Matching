...
class TMFXMLParser:
    ...
    def _parse_entity_element(self, element: ET.Element) -> Optional[TMFEntity]:
        ...
        if tag_name in ['Class', 'ObjectProperty', 'DatatypeProperty', 'Individual']:
            ...
            entity.id = element.get(f'{{{self.namespace_map["rdf"]}}}about', 
    ...
    def _parse_object_property(self, element: ET.Element, ontology: TMFOntology) -> None:
        property_id = element.get(f'{{{self.namespace_map["rdf"]}}}about', 
                                 element.get(f'{{{self.namespace_map["rdf"]}}}ID', ''))
...
class TMFFormatParser:
    ...
    def _parse_entity_attribute(self, line: str, entity: TMFEntity) -> None:
        ...
        if attr_name == 'description':
            ...
            children = [child.strip().replace(' ', '_').lower() 
            ...
            relationships = [rel.strip().replace(' ', '_').lower() 
                           for rel in attr_value.split(',')]
    ...
    def parse_file(self, file_path: str) -> TMFOntology:
        ...
        try:
            if file_extension in ['.xml', '.owl', '.rdf']:
                ...
                self.logger.warning(f"Unknown file format: {file_extension}. Trying XML parser.")
                return self.xml_parser.parse_file(file_path)
        except Exception as e:
