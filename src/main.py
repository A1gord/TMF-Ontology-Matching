...
class TMFOntologyMatcher:
    ...
    def _setup_alignment_engine(self):
        ...
        self.alignment_engine = TMFAlignmentEngine(
            self.matching_engine, alignment_config
        )
    ...
    def _create_strategy(self, strategy_config):
        ...
        if not strategy_class:
            self.logger.warning(
                f"Unknown strategy type: {strategy_config.strategy_type}"
            )
            return None

        try:
            return strategy_class(
                weight=strategy_config.weight, **strategy_config.parameters
            )
        except Exception as e:
            self.logger.error(
                f"Error creating strategy {strategy_config.strategy_type}: {e}"
            )
            return None

    def match_ontologies(
        self, source_path: str, target_path: str, output_path: str = None
    ):
        try:
            ...
            matching_results = self.matching_engine.match_ontologies(
                source_ontology, target_ontology
            )

            matching_stats = self.matching_engine.get_matching_statistics(
                matching_results
            )
            self.logger.info(
                f"Matching completed: {matching_stats['total_matches']} matches found"
            )
    ...
    def align_ontologies(
        ...
        try:
            ...
            alignment_results = self.alignment_engine.align_ontologies(
                source_ontology, target_ontology
            )

            alignment_stats = self.alignment_engine.get_alignment_statistics(
                alignment_results
            )
            self.logger.info(
                f"Alignment completed: {alignment_stats['total_alignments']} alignments found"
            )
    ...
    def evaluate_results(
        self, results_path: str, ground_truth_path: str, output_path: str = None
    ):
        try:
            import json
            ...
            if "matches" in results_data:
                ...
                raise ValueError(
                    "Results file must contain either 'matches' or 'alignments'"
                )
    ...
    def export_matching_to_neo4j(
        ...
        try:
            ...
            self.neo4j_exporter.export_matching_results(
                matching_results, source_name, target_name
            )
    ...
    def export_alignment_to_neo4j(
        ...
        try:
            ...
            self.neo4j_exporter.export_ontology(source_ontology)
            self.neo4j_exporter.export_ontology(target_ontology)
            self.neo4j_exporter.export_alignment_results(
                alignment_results, source_name, target_name
            )
    ...
    def convert_to_rdf(
        self, ontology_path: str, output_path: str, format: str = "turtle"
    ):
        try:
            self.logger.info(f"Loading ontology from {ontology_path}")
            ontology = self.parser.parse_file(ontology_path)
    ...
    def convert_from_rdf(
        self, rdf_path: str, output_path: str = None, ontology_name: str = None
    ):
        try:
...
def main():
    parser = argparse.ArgumentParser(
        description="TMF Ontology Matching and Alignment Tool"
    )
    parser.add_argument(
        "command",
        choices=[
            ...
            "export-neo4j",
            "import-neo4j",
            "export-matching-neo4j",
            "export-alignment-neo4j",
            "to-rdf",
            "from-rdf",
            "neo4j-to-rdf",
            "rdf-to-neo4j",
    ...
    parser.add_argument(
        "--ground-truth", "-g", help="Ground truth file path for evaluation"
    )
    parser.add_argument("--results", "-r", help="Results file path for evaluation")
    parser.add_argument(
        "--ontology-name", "-n", help="Ontology name for Neo4j operations"
    )
    ...
    parser.add_argument(
        "--format", "-f", default="turtle", help="RDF format (turtle, xml, n3, etc.)"
    )
    ...
    try:
        ...
        if args.command == "match":
            ...
            if not args.results or not args.ground_truth:
                parser.error(
                    "evaluate command requires --results and --ground-truth arguments"
                )
            ...
            if not args.source or not args.target:
                parser.error(
                    "export-matching-neo4j command requires --source and --target arguments"
                )
            matcher.export_matching_to_neo4j(
                args.source, args.target, args.source_name, args.target_name
            )

        elif args.command == "export-alignment-neo4j":
            if not args.source or not args.target:
                parser.error(
                    "export-alignment-neo4j command requires --source and --target arguments"
                )
            matcher.export_alignment_to_neo4j(
                args.source, args.target, args.source_name, args.target_name
            )
            ...
            if not args.ontology_name or not args.output:
                parser.error(
                    "neo4j-to-rdf command requires --ontology-name and --output arguments"
                )
            ...
            if not args.source or not args.ontology_name:
                parser.error(
                    "rdf-to-neo4j command requires --source and --ontology-name arguments"
                )
