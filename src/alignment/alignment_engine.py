...
class HierarchicalAnalyzer:
    ...
    def analyze_hierarchical_relationship(
        self,
        entity1: TMFEntity,
        entity2: TMFEntity,
        kg1: TMFKnowledgeGraph,
        kg2: TMFKnowledgeGraph,
    ) -> Dict[str, Any]:
        ...
        common_descendants = self._find_common_descendant_patterns(
    ...
    def _calculate_hierarchical_distance(
        self,
        entity1: TMFEntity,
        entity2: TMFEntity,
        kg1: TMFKnowledgeGraph,
    ...
    def _find_common_ancestor_patterns(
        self, ancestors1: List[TMFEntity], ancestors2: List[TMFEntity]
    ) -> List[Tuple[str, str]]:
        patterns = []
    ...
    def _find_common_descendant_patterns(
        self, descendants1: List[TMFEntity], descendants2: List[TMFEntity]
    ) -> List[Tuple[str, str]]:
        patterns = []
...
class StructuralPropagator:
    ...
    def propagate_alignments(
        self,
        initial_alignments: List[AlignmentResult],
        kg1: TMFKnowledgeGraph,
        kg2: TMFKnowledgeGraph,
        ...
        alignment_map = {
            (a.source_entity.id, a.target_entity.id): a for a in initial_alignments
        }
    ...
    def _propagate_from_alignment(
        self,
        base_alignment: AlignmentResult,
        kg1: TMFKnowledgeGraph,
        kg2: TMFKnowledgeGraph,
        existing_alignments: Dict[Tuple[str, str], AlignmentResult],
    ) -> List[AlignmentResult]:
        ...
        for source_neighbor in source_neighbors:
            for target_neighbor in target_neighbors:
                ...
                if alignment_key not in existing_alignments:
                    propagated_confidence = base_alignment.confidence * (
                        1 - self.config.confidence_decay_factor
                    )
    ...
    def _deduplicate_alignments(
        ...
        for alignment in alignments:
            ...
            if (
                key not in unique_alignments
                or alignment.confidence > unique_alignments[key].confidence
            ):
...
class TMFAlignmentEngine:
    def __init__(
        self, matching_engine: TMFMatchingEngine, config: AlignmentConfiguration = None
    ):
    ...
    def align_ontologies(
        self, source_ontology: TMFOntology, target_ontology: TMFOntology
    ) -> List[AlignmentResult]:

        self.logger.info(
            f"Starting ontology alignment: {source_ontology.name} -> {target_ontology.name}"
        )
        ...
        self.logger.info(
            f"Alignment completed. Generated {len(alignments)} alignments."
        )
        return alignments

    def _create_alignment_from_match(
        self,
        match: MatchingResult,
        source_kg: TMFKnowledgeGraph,
        target_kg: TMFKnowledgeGraph,
    ) -> Optional[AlignmentResult]:

        hierarchical_analysis = None
        if self.config.enable_hierarchical_analysis:
            hierarchical_analysis = (
                self.hierarchical_analyzer.analyze_hierarchical_relationship(
        ...
        hierarchical_distance = (
            hierarchical_analysis.get("hierarchical_distance", 0.5)
    ...
    def _determine_alignment_type(
        self, similarity_score: float, hierarchical_analysis: Optional[Dict[str, Any]]
    ) -> AlignmentType:
    ...
    def _adjust_confidence_with_hierarchy(
        self,
        original_confidence: float,
        hierarchical_analysis: Optional[Dict[str, Any]],
    ) -> float:
    ...
    def _extract_structural_features(
        ...
        return {
            ...
            "property_count": (len(entity1.properties), len(entity2.properties)),
            "relationship_count": (
    ...
    def _filter_and_rank_alignments(
        self, alignments: List[AlignmentResult]
    ) -> List[AlignmentResult]:
    ...
    def get_alignment_statistics(
        ...
        avg_hierarchical_distance = (
            sum(a.hierarchical_distance for a in alignments) / total_alignments
        )
        avg_structural_similarity = (
            sum(a.structural_similarity for a in alignments) / total_alignments
        )
    ...
    def create_alignment_mapping(
        ...
        for alignment in alignments:
            if alignment.alignment_type in [
                AlignmentType.EQUIVALENCE,
                AlignmentType.SUBSUMPTION,
            ]:
